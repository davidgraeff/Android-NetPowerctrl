        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mIsEditMode && isEnabled()) {
                    ...
                    mCurrentPosition = pointToPosition(mDownX, mDownY);
                    int itemNum = mCurrentPosition - getFirstVisiblePosition();
                    View selectedView = getChildAt(itemNum);
                    if (selectedView == null) {
                        return false;
                    } else {
                        mStartClickTime = System.currentTimeMillis();
                        mMobileItemId = getAdapter().getItemId(mCurrentPosition);
                        mHoverCell = getAndAddHoverView(selectedView);
                        if (isPostHoneycomb())
                            selectedView.setVisibility(View.INVISIBLE);
                        mCellIsMobile = true;
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mIsEditMode && !mReorderAnimation && mUserItemClickListener != null) {
                    ...
                    if (clickDuration < MAX_CLICK_DURATION) {
                        ...
                        if (view == null) {
                            ...
                            mUserItemClickListener.onItemClick(this, view, mCurrentPosition, id);
                        }
                    }
                }
                break;
        ...
        if (isForward) {
            for (int pos = Math.min(oldPosition, newPosition); pos < Math.max(oldPosition, newPosition); pos++) {
                ...
                if ((pos + 1) % getNumColumns() == 0) {
                    resultList.add(createTranslationAnimations(view, -view.getWidth() * (getNumColumns() - 1), 0, view.getHeight(), 0));
                } else {
