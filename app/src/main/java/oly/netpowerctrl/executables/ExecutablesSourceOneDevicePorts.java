 ...
 */
public class ExecutablesSourceOneDevicePorts extends ExecutablesSourceBase implements onCollectionUpdated<Object, Device>, onDataQueryCompleted, onDataLoaded {
    private List<DevicePort> mList = new ArrayList<>();
    private Device device;

    public ExecutablesSourceOneDevicePorts(@Nullable ExecutablesSourceChain executablesSourceChain, @NonNull Device device) {
    ...
    public boolean updated(@NonNull Object collection, Device device, @NonNull ObserverUpdateActions action, int position) {
        if (adapterWeakReference == null || device == null || device != this.device)
            return true;
